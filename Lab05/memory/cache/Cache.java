package COA2023.Lab05.memory.cache;

import COA2023.Lab05.memory.Memory;
import COA2023.Lab05.memory.cache.cacheReplacementStrategy.ReplacementStrategy;
...
public class Cache {
    ...
    private int fetch(String pAddr) {
        ...
        if ( rowNO == -1 ) {
            if ( replacementStrategy == null ) {
                rowNO = Integer.parseInt(pAddr.substring(tagBinaryLen, tagLen), 2);
                update(rowNO, pAddr.substring(0, tagBinaryLen).toCharArray(), Memory.getMemory().read(pAddr.substring(0, tagLen) + "000000", LINE_SIZE_B));
            }
            else {
                rowNO = replacementStrategy.replace(setNO * setSize, (setNO + 1) * setSize - 1,
                        pAddr.substring(0, tagBinaryLen).toCharArray(), Memory.getMemory().read(pAddr.substring(0, tagLen) + "000000", LINE_SIZE_B));
            }
        }
        else {
    ...
    private int map(int blockNO) {
        ...
        for ( int i = setNO * setSize; i < (setNO + 1) * setSize; i++ ) {
            if ( !cache[i].validBit ) continue;
            if ( Integer.parseInt(new String(cache[i].tag), 2) == tag ) {
                return i;
            }
        }
        return -1;
    ...
    public void update(int rowNO, char[] tag, byte[] input) {
        ...
        for ( int i = 0; i < tag.length; i++ ) {
            cache[rowNO].tag[i + tagLen - tag.length] = tag[i];
        }
